...
class TestStellarBurgersRegistration:
    ...
    def test_registration_page_successful_registration(self, driver): # Поля формы регистрации заполнены валидными данными
        ...
        driver.find_element(*Registration.re_name).send_keys(PersonData.name) # Найти поле Имя и заполнить
        driver.find_element(*Registration.re_email).send_keys(PersonData.login) # Найти поле Email и заполнить
        driver.find_element(*Registration.re_password).send_keys(PersonData.password) # Найти поле Пароль и заполнить
    ...
    def test_registration_password_error(self, driver, incorrect_password): # В поле Пароль формы регистрации менее 5 символов
        ...
        driver.find_element(*Registration.re_name).send_keys('Дейнерис') # Найти поле Имя и заполнить
        driver.find_element(*Registration.re_email).send_keys('alexandra_tribun_16_169@ya.ru') # Найти поле Email и заполнить
